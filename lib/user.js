                    ...
                    if (entityType == 'Attendee') {
                        ...
                        if (payload.data?.includeStat) {
                            ...
                            if (regType === 'Sponsor') {
                                qnaFilter = {
                                    ice_id: iceId,
                                    entity_id: regTypeEntityId,
                                    entity_type: regType.toLowerCase()
                                };

                                meetingOrFilter = `
                                                or=(
                                                and(requestor_type.eq.${entityType},requestor_type_entity_id.eq.${regTypeEntityId}),
                                                and(invitee_type.eq.${entityType},invitee_type_entity_id.eq.${regTypeEntityId})
                                                )
                                            `;
                            } else {
                                qnaFilter = {
                                ...
                                meetingOrFilter = `
                                            or=(
                                            and(requestor_type.eq.${entityType},requestor_id.eq.${entityId}),
                                            and(invitee_type.eq.${entityType},invitee_id.eq.${entityId})
                                            )
                                        `;
                            }
                            ...
                            const qnaSet = new Set((qnaData || []).map(q => q.question_id));
                            const totalQnA = qnaSet.size;

                            // --- Meeting Stats Query ---
                            const { data: meetingData, error: meetingError } = await supabase
                                .from('meeting')
                                .select('request_status, is_created_by_ai')
                                .eq('ice_id', iceId)
                            ...
                            for (const row of meetingData || []) {
                                if (row.request_status === 'draft') meetingStats.draftCount++;
                                if (row.request_status === 'requested') meetingStats.requestedCount++;
                                if (row.request_status === 'confirmed') meetingStats.confirmedCount++;
                                if (row.is_created_by_ai) meetingStats.aiMatchCount++;
                            }
                        ...
                        if (payload.data?.includeStat) {
                            ...
                            const { data: meetingData, error: meetingError } = await supabase
                                ...
                                .select('request_status, is_created_by_ai')
                                .eq('ice_id', iceId)
                                .or(`and(requestor_type.eq.${entityType},requestor_type_entity_id.eq.${entityId}),
                                and(invitee_type.eq.${entityType},invitee_type_entity_id.eq.${entityId})`);
                            ...
                            for (const meeting of meetingData || []) {
                                if (meeting.request_status === 'draft') meetingStats.draftCount++;
                                if (meeting.request_status === 'requested') meetingStats.requestedCount++;
                                if (meeting.request_status === 'confirmed') meetingStats.confirmedCount++;
                                if (meeting.is_created_by_ai) meetingStats.aiMatchCount++;
                            }
            ...
            if (matchingAttendees.length > 0) {
                const totalScansBySponsor = matchingAttendees.reduce((sum, att) => {
                    ...
                    return sum + scannedList.reduce((innerSum, entry) => {
                        return innerSum + (entry?.Attendee?.length || 0);
                    }, 0);
                }, 0);
