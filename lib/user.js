                ...
                if (entityData) {
                    if (entityType == 'Attendee') {
                        ...
                        WHERE iceId = ? AND entityId = ? AND entityType = ?`;
                            ...
                            const qnaQueryParams = [iceId, entityId, regType.toLowerCase()];
...
async function user_update(payload) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const allowedUserFields = ["FirstName", "LastName", "Name", "Address", "Company", "Designation", "Phone", "isHiddenFromChat", "preferredSlots"];
