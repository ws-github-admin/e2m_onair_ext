...
async function get_sponsors_meeting(payload) {
    ...
    try {
        ...
        if (!meetingSnapshot.empty) {
            ...
            doc.moveDown(0.3);  // Reduced from 1 to bring them slightly closer

            // Event name - Darker gray with slightly increased size
            doc.fontSize(16)  // Increased from 10 to 12
                .font('Helvetica')
                .fillColor('#444444')  // Darker gray (75% black instead of 60%)
            ...
            for (const sponsorId in groupedData) {
                ...
                if (meetings.length > 0) {
                    ...
                    meetings.forEach(meeting => {
                        ...
                        doc.font('Helvetica')
                            .fontSize(12)
                            .fillColor('#1565C0') // More professional blue
                            .text(`Time Slot: ${slot}`); // Added underline
                        doc.moveDown(0.5);

                        const boxWidth = 250, boxHeight = 110, startX = doc.x, startY = doc.y;

                        // Requestor Box - enhanced styling
                        doc.rect(startX, startY, boxWidth, boxHeight)
                            .lineWidth(0.5)
                            .stroke('#333333'); // Darker border


                        doc.fontSize(10)
                            .font('Helvetica-Bold')
                            .fillColor('#333333') // Dark text
                            .text('REQUESTOR', startX + (boxWidth - doc.widthOfString('REQUESTOR')) / 2, startY + 5);

                        // Content
                        doc.font('Helvetica')
                            .fillColor('#444444') // Darker text for better readability
                            .text(`Name: ${requestor.Name || 'N/A'}`, startX + 10, startY + 25)
                            .text(`Company: ${requestor.Company || 'N/A'}`, startX + 10, startY + 40)
                            .text(`Designation: ${requestor.Designation || 'N/A'}`, startX + 10, startY + 55)
                            .text(`Email: ${requestor.Email || 'N/A'}`, startX + 10, startY + 70)
                            .text(`Phone: ${requestor.Phone || 'N/A'}`, startX + 10, startY + 85);

                        // Invitee Box - matching enhanced styling
                        const inviteeX = startX + boxWidth + 20;
                        ...
                        doc.font('Helvetica')
                            ...
                            .text(`Company: ${invitee.Company || 'N/A'}`, inviteeX + 10, startY + 40)
                            .text(`Designation: ${invitee.Designation || 'N/A'}`, inviteeX + 10, startY + 55)
