...
async function user_info(payload) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const attendeeDoc = await dbClient.collection(iceId).doc("AttendeeList").collection("Attendees").doc(entityId).get();

            if (attendeeDoc.exists) {
                const attendeeData = attendeeDoc.data();
                const regType = attendeeData?.RegistrationType?.RegistrationType?.toLowerCase();
                const regTypeEntityId = attendeeData?.RegistrationType?.RegistrationTypeEntityId;
                console.log("regType", regType);
                console.log("regTypeEntityId", regTypeEntityId);

                if (regType === "sponsor" && payload.data.attendeeInfoOnly !== true) {
                    entityId = regTypeEntityId;
                }

                let entityType = (regType === "sponsor" && payload.data.attendeeInfoOnly !== true) ? "Sponsor" : "Attendee";


                const entityCollectionPath = `${eventBasePath}/${entityType}List/${entityType}s`;
                ...
                const userData = await _get_from_cache_or_db(cacheKey, async () => {
                    if (entityId) {
                ...
                if (!userData) return reject(ERRCODE.DATA_NOT_FOUND);
                ...
                if (payload.data?.includeStat) {
                    ...
                    const qnaQueryParams = [iceId, entityId, entityType];
                ...
                ret_val.status = 0;
                ret_val.result = _fields(userData, payload.data.fields);
            }
            // Final response
            resolve(ret_val);
        } catch (err) {
