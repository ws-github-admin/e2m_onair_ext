            ...
            if (entityId) {
                ...
                if (entityData) {
                    ...
                    if (entityType == 'Attendee') {
                        ...
                        const sessions = [];
                        const sessionSnapshot = await dbClient.collection(`${eventBasePath}/SessionList/Sessions`).get();

                        sessionSnapshot.forEach(doc => {
                            const sessionData = doc.data();
                            const speakers = Array.isArray(sessionData?.MappedSpeakers) ? sessionData.MappedSpeakers : [];

                            const isMatched = speakers.some(s => s.AttendeeId === entityId);

                            if (isMatched) {
                                const { MappedSpeakers, ...rest } = sessionData;
                                sessions.push({
                                    ...rest
                                });
                            }
                        ...
                        ret_val.result.Sessions = sessions;
                    }
                    else if (entityType == 'Session') {
                        ret_val.status = 0;
                        ret_val.result = _fields(entityData, payload.data.fields);
        ...
        try {
            ...
            if ('preferredSlots' in sanitizedUserUpdates) {
                ...
                const { error } = await supabase
                    ...
                    .upsert(
                        [{
                            attendeeId: pa.UserId,
                            slots: preferredSlots
                        }],
                        { onConflict: ['attendeeId'] } // assumes attendeeId is unique or part of a unique constraint
                    );
...
async function get_attendance(payload) {
    return new Promise(async (resolve, reject) => {
        let ret_val = { status: -1 };
        try {
            logger.log(payload);
            ...
            const pd = payload.data || {};
            const eventBasePath = pk.instanceId + "_" + pk.clientId + pk.eventId;
            const userPath = `${eventBasePath}/AttendeeList/Attendees/${pd.uid}`;
            const userDocRef = dbClient.doc(userPath);
            const userDoc = await userDocRef.get();
            ...
            ret_val = { 
                ...
                data: {
                    uid: pd.uid,
                    Attending: userData.Attending || false,
                    // Include other attendance-related fields if needed
                    // RegistrationType: userData.RegistrationType,
                    // AttendeeStatus: userData.AttendeeStatus,
                }
            };
...
async function update_attendance(payload) {
    return new Promise(async (resolve, reject) => {
        let ret_val = { status: -1 };
        try {
            logger.log(payload);
            ...
            // Update Attending field only if pd.Attending == true
            if (typeof pd.Attending === 'boolean') {
                await userDocRef.update({
                    Attending: pd.Attending
                });
                
                ret_val = { 
                    status: 1, 
                    message: `Attendee marked as ${pd.Attending ? 'attending' : 'not attending'}`,
                    data: {
                        uid: pd.uid,
                        Attending: pd.Attending
                    }
                ...
                ret_val = { 
                    status: 0, 
                    message: "No update required - Attending value is not a boolean" 
                };
            }
