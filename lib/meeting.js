...
async function mark_meeting_attended(payload) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            if (!payload.data.Attended && false) {
                const [clientSnap, eventSnap] = await Promise.all([
                    dbClient.doc(`${instance_base_path}/ClientList/Clients/${payload.key.clientId}`).get(),
                    dbClient.doc(`${event_base_path}/EventInfo`).get()
                ]);
                ...
                const placeholders = {
                    ...commonPlaceholders,
                    SenderCompanyName: Meeting.Requestor.Company || "",
                    SenderName: Meeting.Requestor.Name || "",

                    ReceiverName: Meeting.Invitee.Name || "",
                    Title: Meeting.Invitee.Title || "",
                    Company: Meeting.Invitee.Company || "",
