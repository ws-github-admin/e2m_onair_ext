...
async function request_meetings(payload) {
    ...
    const MAX_CONFIRM = RequestorIsSponsor ? config.ATTENDEE_MAX_CONFIRM_REQUEST : config.ATTENDEE_MAX_CONFIRM_REQUEST;
...
async function consolidated_send_email(payload) {
    ...
    const meetingResults = await mysql.fetchQuery(`
        SELECT * FROM e2m_o2o_prd.meeting 
        WHERE iceId = ? AND sendEmail = true AND status = 'requested'
    `, [iceId]);

    const attendeeMap = {};  // Map of attendeeId -> meetings

    for (const meeting of meetingResults) {
        const attendeeId = meeting.InviteeId;
        if (!attendeeMap[attendeeId]) attendeeMap[attendeeId] = [];
        attendeeMap[attendeeId].push(meeting);
    }

    const tasks = [];

    for (const [attendeeId, meetings] of Object.entries(attendeeMap)) {
        const attendeeDoc = await dbClient.collection(`${event_base_path}/AttendeeList/Attendees`).doc(attendeeId).get();
        ...
        const linksList = await Promise.all(meetings.map(async meeting => {
            ...
            if (!sponsor) {
                const sponsorDoc = await dbClient.doc(`${event_base_path}/SponsorList/Sponsors/${meeting.RequestorId}`).get();
                sponsor = sponsorDoc.exists ? sponsorDoc.data() : {};
                sponsorCache[meeting.RequestorId] = sponsor;
            }

            const sponsorName = sponsor.Name || meeting.RequestorName || "Sponsor";
            const sponsorCompany = sponsor.Company || "Company";
            const url = `https://meet.e2m.live?mid=${meeting.MeetingId}&email=${attendee.Email}&sid=${meeting.RequestorId}&eid=${eventId}`;
            return `<li><a href="${url}">Meeting with ${sponsorName} (${sponsorCompany})</a></li>`;
        }));

        const emailBody = `
            <p>Hi ${attendee.Name},</p>
            <p>You have new meeting requests at ${event.EventGroupName}. Please review them below:</p>
        ...
        if (attendee.Email && attendee.Email.endsWith('@webspiders.com')) {
            tasks.push(_send_email({}, EmailPayload));
        }

        // Step 3: Mark all these meetings' sendEmail = false
        const meetingIds = meetings.map(m => `'${m.MeetingId}'`).join(',');
        ...
        for (const { role, data, counterpart } of recipients) {
            ...
            try {
                const meetingTime = Meeting.Timezone
                    ? moment.tz(Meeting.Slots[0], 'UTC').tz(Meeting.Timezone)
                    : moment(Meeting.Slots[0]).utc();

                const icsPayload = {
                    start: [meetingTime.year, meetingTime.month, meetingTime.day, meetingTime.hour, meetingTime.minute],
                    duration: { minutes: configData.Duration || 10 },
                    title: emailSubject,
                    description: htmlToText(emailBody, { wordwrap: 130 }),
                    location: "UK",
                    status: "CONFIRMED",
