...
async function meeting_attendees(payload) {
    ...
    for (const meeting of meetings) {
        ...
        if (sponsorId) {
            ...
            attendeeId = meeting.inviteeTypeEntityId === sponsorId ? meeting.requestorId : meeting.inviteeId;
        } else {
...
async function on_sms_replied(payload) {
    try {
        ...
        if (payload.To == '+19295520827') {
            sms_body = `E2M Alert! Received message from [${payload.From}]:
---
${payload.Body}`;

            tasks = [];
            for (let number of to_numbers) {
                tasks.push(_send_sms({ to: number, message: smsText }));
            }
