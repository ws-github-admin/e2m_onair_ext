...
async function ai_confirm_meeting(payload) {
    ...
    const result = isSponsor 
        ? await _confirmed_meetings_slots_by_sponsors(iceId)
        : await _confirmed_meetings_slots_by_attendees(iceId);
    
    console.log(isSponsor ? "sponsor result" : "attendee result", result);
    
    const confirmedSlotsMap = result.confirmedSlotsMap;
    const matchSlotsMap = result.preferredSlotsMap;
    
    // Get requestor's preferred and confirmed slots
    const requestorEntityId = isSponsor ? sponsorId : requestorId;
    const preferred = matchSlotsMap[requestorEntityId] || [];
    const confirmed = confirmedSlotsMap[requestorEntityId] || [];
    ...
    const confirmedCounts = matchType === "Sponsor" 
    ...
    for (const [entityId, slots] of Object.entries(confirmedSlotsMap)) {
    ...
    for (const { id: matchId } of scoredMatches) {
        ...
        if (!match || match.confirmed >= matchLimit) continue;

        const matchMaxAllowed = matchLimit - match.confirmed;
        if (matchMaxAllowed <= 0) continue;

        const matchSlots = matchSlotsMap[matchId] || [];
        const matchConfirmedArray = confirmedSlotsMap[matchId] || [];
        const requestorConfirmedArray = confirmedSlotsMap[requestorEntityId] || [];
        
        // Ensure we have Sets for efficient lookup
        const matchConfirmedSlots = new Set(matchConfirmedArray);
        const requestorConfirmedSlots = new Set(requestorConfirmedArray);

        console.log("matchSlots", matchSlots);
        console.log("matchConfirmedSlots", Array.from(matchConfirmedSlots));

        const validSlot = requestorAvailableSlots.find(slot =>
            matchSlots.includes(slot) &&
            !requestorConfirmedSlots.has(slot) &&
