...
async function available_attendees(payload) {
    ...
    try {
        ...
        const requestedMeetings = await mysql.executeQuery(
            `SELECT inviteeId FROM meeting WHERE requestorTypeEntityId = ? AND requestStatus IN ('requested','cancelled')`,
            [registrationTypeEntityId]
        );
        let requestedIds = new Set();
        if (requestedMeetings && requestedMeetings.length > 0) {
            requestedIds = new Set(requestedMeetings.map(row => row.inviteeId));
        }

        const draftedMeetings = await mysql.executeQuery(
            `SELECT inviteeId FROM meeting WHERE requestorTypeEntityId = ? AND requestStatus ='draft'`,
            [registrationTypeEntityId]
        );
        let draftedIds = new Set();
        if (draftedMeetings && draftedMeetings.length > 0) {
            draftedIds = new Set(draftedMeetings.map(row => row.inviteeId));
        }
...
async function request_meetings(payload) {
    ...
    if (!RequestorIsSponsor) {
        ...
        for (const sponsorId of inviteeIds) {
            ...
            let minMeetings = repUsageCounts[reps[0]] || 0;

            for (let i = 1; i < reps.length; i++) {
                const count = repUsageCounts[reps[i]] || 0;
        ...
        // Normal case
        const inviteeDocs = await Promise.all(inviteeIds.map(id =>
    ...
    const MAX_CONFIRM = RequestorIsSponsor ? config.SPONSOR_MAX_CONFIRM : config.ATTENDEE_MAX_CONFIRM;

    // if (invitees.length < MIN_REQUESTS) {
    //     ret_val.message = `Minimum ${MIN_REQUESTS} request required.`;
    //     return ret_val;
    // }
    // if (invitees.length > MAX_REQUESTS) {
    //     ret_val.message = `Maximum ${MAX_REQUESTS} request allowed.`;
    //     return ret_val;
    // }
    ...
    const meetingPromises = validAttendees.map(async (invitee) => {
        ...
        try {
            ...
            return { success: true, meetingData: meetingDoc.data() };
        } catch (err) {
...
async function pubsub_request_meeting(pubsubPayload) {
    ...
    try {
        ...
        let confirm_meeting = pubsubPayload.confirm_meeting ? pubsubPayload.confirm_meeting : false
        ...
        console.log("confirm_meeting", confirm_meeting)

        if (confirm_meeting) {
            await confirm_meeting({
                key: payload.key,
                data: { MeetingId: Meeting.MeetingId, Slot: payload.data.Slot },
                auth: payload.auth
            });
            ret_val.status = 0
            ret_val.message = `Meeting ${Meeting.MeetingId} confirmed and notifications sent`
        } else {
            ...
            const MeetingUrl = `https://meet.e2m.live?mid=${Meeting.MeetingId}&email=${Invitee.Email}&sponsor=${Sponsor.SponsorId}`;
            ...
            for (const { role, data, counterpart } of recipients) {
                ...
                if (data.Email && data.Email.endsWith('@webspiders.com')) {
                    tasks.push(_send_email(Client, EmailPayload));
                }

                // Send SMS if enabled and phone is available
                if (configData.SendSMS && data.Phone) {
                    const smsText = `${counterpart.Name}, ${counterpart.Designation} of ${counterpart.Designation} requested a Meeting with you`;
...
async function pubsub_confirm_meeting(pubsubPayload) {
    ...
    try {
        ...
        for (const { role, data, counterpart } of recipients) {
            ...
            // Send SMS if enabled and phone is available
            if (configData.SendSMS && data.Phone && data.Email.endsWith('@webspiders.com')) {
                const smsText = `Meeting with ${data.Name}, ${data.Designation} of  ${data.Company} has been confirmed on ${startDateTime}`;
                tasks.push(_send_sms({ to: counterpart.Phone, message: smsText }));
                console.log("SMS to be sent:", smsText);
            }
        }
        tasks.push(_attach_meeting_reminders(payload.key, Meeting))
...
async function meeting_reminder(req, res) {
    try {
        ...
        for (const { role, data, counterpart } of recipients) {
            ...
            if (meeting.SendSMS && data.Phone && data.Email.endsWith('@webspiders.com')) {
                const smsText = `Reminder: Meeting with ${counterpart.Name}, ${counterpart.Designation} of ${counterpart.Company} on ${startDateTime}. Please click the following to accept. ${MeetingUrl}`;
...
async function _confirmed_meetings_slots_by_sponsors(iceId) {
    const config = await firestore.getConfig(iceId); // Assuming you store config under MeetingList/Settings/Config
    const allSlots = config?.Slots || [];
...
async function _confirmed_meetings_slots_by_attendees(iceId) {
    ...
    const result = await mysql.queryExecute(query);
...
async function _sponsor_representatives_map(eventPath, sponsorIds) {
    ...
    sponsorDocs.forEach(doc => {
        if (doc.exists) {
            ...
            sponsorMap[sponsor.SponsorId] = sponsor.Representatives || [];
        }
    });
