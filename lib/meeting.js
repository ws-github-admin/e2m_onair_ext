...
async function request_meetings(payload) {
    ...
    if (!RequestorIsSponsor) {
        ...
        for (const sponsorId of inviteeIds) {
            ...
            //         requestorId, requestorId
            //     ], true)
            const { data: confirmedMeetings, error } = await supabase
                .from('meeting')
                .select('*')
                .or(`
                    and(
                    or("requestorId".eq.${requestorId},"requestorTypeEntityId".eq.${requestorId}),
                    or("inviteeId".eq.${sponsorId},"inviteeTypeEntityId".eq.${sponsorId})
                    ),
                    and(
                    or("requestorId".eq.${sponsorId},"requestorTypeEntityId".eq.${sponsorId}),
                    or("inviteeId".eq.${requestorId},"inviteeTypeEntityId".eq.${requestorId})
                    )
                `)
            ...
            if (confirmedMeetings && confirmedMeetings.length >= 1) {
        ...
        for (const inviteeId of inviteeIds) {
            ...
            const { data: confirmedMeetings, error } = await supabase
                ...
                .or(`
                    and(
                    or("requestorId".eq.${registrationTypeEntityId},"requestorTypeEntityId".eq.${registrationTypeEntityId}),
                    or("inviteeId".eq.${inviteeId},"inviteeTypeEntityId".eq.${inviteeId})
                    ),
                    and(
                    or("requestorId".eq.${inviteeId},"requestorTypeEntityId".eq.${inviteeId}),
                    or("inviteeId".eq.${registrationTypeEntityId},"inviteeTypeEntityId".eq.${registrationTypeEntityId})
                    )
                `)
                .eq('"requestStatus"', 'confirmed');
    ...
    let MIN_REQUESTS = 0;
    console.log("RequestorIsSponsor", RequestorIsSponsor);
    if (RequestorIsSponsor) {
    ...
    console.log("MAX_CONFIRM", MAX_CONFIRM);
    ...
    const checkResults = await Promise.allSettled(invitees.map(async (invitee) => {
        ...
        const { data: confirmedMeetings, error } = await supabase
            ...
            .eq('"iceId"', iceId)
            .eq('"requestStatus"', 'confirmed')
            .or(`"requestorId".eq.${invitee.AttendeeId},"inviteeId".eq.${invitee.AttendeeId}`);

        if (confirmedMeetings && confirmedMeetings.length >= MAX_CONFIRM) {
            return { skipped: invitee.AttendeeId };
        }
    ...
    for (const result of checkResults) {
        console.log("fulfilled", result.status);
        if (result.status === "fulfilled") {
            ...
            if (!RequestorIsSponsor) {
                ...
                const { data, error } = await supabase
                    ...
                    .upsert(
                        ...
                        {
                            onConflict: 'meetingCode', // this must be UNIQUE in Supabase
                            ignoreDuplicates: false     // ensures it updates on conflict
                        }
                    );
                if (error) console.log("create meeting supabase error:", error)
                    ...
                    .upsert(
                        {
                            ...
                            "inviteeTypeEntityId": '', // or null if that's your convention
                            "requestStatus": 'requested',
                            "sendEmail": 1,
                            "requestUpdateDateTime": now,
                            "isCreatedByAI": IsCreatedByAI
                        ...
                        {
                            onConflict: 'meetingCode', // Ensure this column has a UNIQUE constraint
                            ignoreDuplicates: false     // Make sure Supabase performs an update if a conflict occurs
                        }
                    );
...
async function pubsub_request_meeting(pubsubPayload) {
    ...
    try {
        ...
        if (confirm_meeting_flag) {
            ...
            const { data, error } = await supabase
                .from('meeting')
                .update({ "sendEmail": 1 }) // use `true` for boolean field
                .in('"meetingCode"', Meeting.MeetingId); // assuming Meeting.MeetingId is an array
