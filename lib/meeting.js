...
async function available_attendees(payload) {
    ...
    try {
        ...
        const draftedMeetings = await mysql.executeQuery(
            `SELECT inviteeId FROM meeting WHERE requestorTypeEntityId = ? AND requestStatus IN ('draft','requested','confirmed','cancelled')`,
            [registrationTypeEntityId]
        );
...
async function pubsub_request_meeting(pubsubPayload) {
    ...
    try {
        ...
        if (confirm_meeting) {
            ...
            const MeetingUrl = `${customDomain}mymeetings?email=${Invitee.Email}&mid=${Meeting.MeetingId}`;
...
function accept_meeting(payload) {
    return new Promise(async (resolve, reject) => {
        logger.log(payload)
        let ret_val = { status: -1 };
...
async function ai_confirm_meeting(payload) {
    ...
    if (isSponsor) {
        ...
        const result = await _confirmed_meetings_slots_by_attendee(iceId);
        confirmedSlotsMap = result.confirmedSlotsMap;
...
async function cancel_meeting(payload) {
    return new Promise(async (resolve, reject) => {
      let ret_val = { status: -1 };
      try {
        if (!payload?.key || !payload?.data?.MeetingId) {
          return reject({ status: -1, message: "Invalid payload." });
        }
        ...
        const meetingRef = dbClient
          .collection(event_base_path)
          .doc("MeetingList")
          .collection("Meetings")
          .doc(payload.data.MeetingId);
  
        const meetingSnap = await meetingRef.get();
        if (!meetingSnap.exists) {
          return reject({ status: -1, message: "Meeting not found." });
        }
...
async function _confirmed_meetings_slots_by_attendee(iceId) {
    const rows = await mysql.executeQuery(`
        SELECT 
            CASE 
    ...
    const attendeeDocs = await firestore.getAllDocuments("Attendees", iceId);
    const preferredSlotsMap = {};

    for (const doc of attendeeDocs) {
        const attendeeId = doc.id;
        const preferred = doc.data().preferredSlots || [];
        preferredSlotsMap[attendeeId] = preferred;
    }
