...
//     const eventIds = eventId === "E1742214690559" ? [iceId] : predefinedEvents.map(e => `${instanceId}_${clientId}${e}`);
...
//     let meetingQuery = ``;
//     let replacements = eventIds;

//     if (registrationType === 'sponsor') {
//         meetingQuery = `
//         SELECT *, iceId AS EventId,
//           CASE 
//             WHEN (requestorTypeEntityId = ? AND requestStatus = 'requested') THEN 'FYI'
//             WHEN (inviteeTypeEntityId = ? AND requestStatus = 'requested') THEN 'FYA'
//             ELSE ''
...
//         replacements.push(registrationTypeEntityId, registrationTypeEntityId);
...
//         replacements.push(current_uid, current_uid);
//     }
...
//     const results = await Promise.all(rows.map(async (row) => {
//         const [requestorDoc, inviteeDoc] = await Promise.all([
//             dbClient.collection(`${eventPath}/AttendeeList/Attendees`).doc(row.requestorId).get(),
//             dbClient.collection(`${eventPath}/AttendeeList/Attendees`).doc(row.inviteeId).get(),
//         ]);
