...
async function request_meetings(payload) {
    ...
    const MAX_CONFIRM_REQUESTOR = RequestorIsSponsor ? config.SPONSOR_MAX_CONFIRM_REQUEST : config.ATTENDEE_MAX_CONFIRM_REQUEST;
    const MAX_CONFIRM_INVITEE = RequestorIsSponsor ? config.ATTENDEE_MAX_CONFIRM_REQUEST : config.SPONSOR_MAX_CONFIRM_REQUEST;
    ...
    const existingMeetingPairs = await _getExistingMeetingPairs(iceId, requestorId, inviteeIds || []);
    ...
    const meetingPromises = validInvitees.map(async (invitee, index) => {
        ...
        if (payload.data.Slots && payload.data.Slots.length > 0) {
            ...
            if (targetSponsorId) {
                try {
                    // Use enhanced slot selection with batch tracking
                    assignedSlot = await _selectOptimalSlot(
                        iceId,
                        targetSponsorId,
                        payload.data.Slots,
                        RequestorIsSponsor ? invitee.AttendeeId : requestor.AttendeeId,
                        batchId
                    );
        ...
        try {
            ...
            if (!RequestorIsSponsor) {
                ...
                const meetingRecord = {
                    ...
                    "requestMeetingSlot": assignedSlot, // Store the assigned slot
                    "batchId": batchId // Store batch ID for correlation
                };
 ...
 */
async function _getExistingMeetingPairs(iceId, requestorId, inviteeIds) {
    try {
...
async function _validateMeetingEligibility(
    ...
    try {
        ...
        const pairKey1 = `${requestorId}-${inviteeId}`;
        const pairKey2 = `${inviteeId}-${requestorId}`;
        ...
        if (requestorIsSponsor) {
            ...
            const sponsorEntityId = inviteeId; // In this case, inviteeId IS the sponsor entity ID
            ...
            const sponsorReps = await _getSponsorRepresentatives(iceId, sponsorEntityId, eventPath);
            ...
            return {
                isValid: true,
                inviteeData: bestSponsorRep.selectedRep,
                originalSponsorId: sponsorEntityId,
                selectedRepId: bestSponsorRep.selectedRep.AttendeeId,
                availableSlots: bestSponsorRep.availableSlots
            };
        }
...
async function _selectBestSponsorRep(
    ...
    try {
        ...
        if (payloadSlots && payloadSlots.length > 0) {
            ...
            eligibleReps = availableReps.map(rep => ({
                ...rep,
                availableSlots: [],
...
async function _selectOptimalSlot(iceId, currentSponsorId, availableSlots, attendeeId, batchId = null) {
    try {
        ...
        // Step 3: Get current sponsor's available slots
        ...
        if (viableSlots.length === 0) {
            ...
            console.warn(`Available slots: ${availableSlots.join(', ')}`);
            console.warn(`Sponsor available: ${currentSponsorAvailableSlots.join(', ')}`);
            console.warn(`Attendee booked: ${Array.from(attendeeBookedSlots).join(', ')}`);
            console.warn(`Batch reserved: ${Array.from(batchReservedSlots).join(', ')}`);
            
            // Try to find any available slot from the original list
        ...
        // Step 8: Sort by final score (highest first)
        scoredSlots.sort((a, b) => b.finalScore - a.finalScore);

        console.log("Slot scoring results:", scoredSlots);

        // Step 9: Return the best slot and reserve it
        const selectedSlot = scoredSlots[0].slot;
        
        // Reserve the selected slot for this batch to prevent conflicts
        if (batchId) {
            batchSlotReservations.get(batchId).add(selectedSlot);
            console.log(`Reserved slot ${selectedSlot} for batch ${batchId}`);
        }
        
        console.log(`Selected slot: ${selectedSlot} for sponsor ${currentSponsorId}`);
        console.log(`Sponsor has ${currentSponsorMeetingCount} existing meetings`);
        console.log(`Selected slot has ${scoredSlots[0].sponsorCountInSlot} sponsor representatives`);
...
async function _save_as_draft(payload) {
    try {
        ...
        if (confirmedMeetings.length > 0 && confirmedMeetings[0].count >= 2) {
