...
async function scan_vcard(payload) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            const allowedEvents = [
                { EventId: "E1742214690559", EventName: "Retail MediaX" },
                { EventId: "E1743163129042", EventName: "Digital Marketing Evolution" },
                { EventId: "E1743163021441", EventName: "Customerx" },
                { EventId: "E1743163201304", EventName: "Subscriptionx" },
                { EventId: "E1743162911584", EventName: "Sustainabilityx" },
                { EventId: "E1743162842566", EventName: "Social Media Masters" },
                { EventId: "E1743162762857", EventName: "Amazon Sellers Summit" },
                { EventId: "E1747670277539", EventName: "FMCG" },
                { EventId: "E1748513963841", EventName: "FMCG Test" },
                { EventId: "E1750747493051", EventName: "WS-Test Event" }
            ];

            const scanTasks = allowedEvents.map(event => {
                const eventPath = `${pk.instanceId}_${pk.clientId}${event.EventId}`;
                return dbClient
                    .collection(`${eventPath}/AttendeeList/Attendees`)
                    .where("VCard.EXT_QRCODE", "==", pd.qrCode)
                    .limit(1)
                    .get()
                    .then(snapshot => ({ snapshot, event }));
            });

            const results = await Promise.all(scanTasks);
            const matchedResults = results.filter(result => !result.snapshot.empty);
            ...
            const scannedData = {
                Attendee: matchedResults.map(result => {
                    const attendee = result.snapshot.docs[0].data();
                    return {
                ...
                Name: matchedResults[0].snapshot.docs[0].data().Name,
                Email: matchedResults[0].snapshot.docs[0].data().Email,
                Designation: matchedResults[0].snapshot.docs[0].data().Designation,
                Company: matchedResults[0].snapshot.docs[0].data().Company,
                Phone: matchedResults[0].snapshot.docs[0].data().Phone,
                Rating: pd.Rating || 0,
                Comment: pd.Comment || '',
                CreatedDateTime: new Date(),
            };

            const scannedSponsorId = matchedResults[0].snapshot.docs[0].data()?.RegistrationType?.RegistrationType?.toLowerCase() === 'sponsor'
                ? matchedResults[0].snapshot.docs[0].data()?.RegistrationType?.RegistrationTypeEntityId
                : null;
            ...
            if (scannedData.Email) {
                ...
                const existingEmails = scannerDoc.exists ? (scannerDoc.data()?.VCard?.Scanned || []).map(s => s.Email) : [];
                if (!existingEmails.includes(scannedData.Email)) {
                ...
                if (sponsorId) {
                    ...
                    if (sponsorDoc.exists) {
                        const sponsorScannedEmails = (sponsorDoc.data()?.VCard?.Scanned || []).map(s => s.Email);
                        if (!sponsorScannedEmails.includes(scannedData.Email)) {
...
async function scan_vcard_3(payload) {
    return new Promise(async (resolve, reject) => {
        let ret_val = { status: -1 };
        try {
            logger.log(payload);
